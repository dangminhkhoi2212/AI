#include <stdio.h>
#include <stdlib.h>
#define capacity_X 14
#define capacity_Y 10
#define capacity_Z 7
#define Empty 0
#define goal 8
...
{
    if (cur_state.x > Empty && cur_state.y < capacity_Y)
    {
        result->x = max(cur_state.x - (capacity_Y - cur_state.y), Empty);
        result->y = min(cur_state.y + cur_state.x, capacity_Y);
...
{
    if (cur_state.x > Empty && cur_state.z < capacity_Z)
    {
...
{
    if (cur_state.y > Empty && cur_state.x < capacity_X)
    {
...
{
    if (cur_state.y > Empty && cur_state.z < capacity_Z)
    {
...
{
    if (cur_state.z > Empty && cur_state.x < capacity_X)
    {
...
{
    if (cur_state.z > Empty && cur_state.y < capacity_Y)
    {
        result->x = cur_state.x;
